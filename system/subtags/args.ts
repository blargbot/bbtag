...
export class ArgsSubtag extends SystemSubtag {
    public constructor() {
        super({
            name: 'args',
            category: 'system',
            arguments: a => [a.o('index'), a.o('range')],
            description:
    ...
    public getArgs(args: ArgumentCollection): SubtagResult {
        const from = this.bbtag.convert.toNumber(args.get(0), NaN);
        const to = args.length === 1 ? from + 1 : this.bbtag.convert.toNumber(args.get(1), v => v === 'n' ? Infinity : NaN);

        if (isNaN(from)) { return this.bbtag.errors.types.notNumber(args, args.getRaw(0)); }
        if (isNaN(to)) { return this.bbtag.errors.types.notNumber(args, args.getRaw(1)); }

        if (from >= args.context.arguments.length) { return this.bbtag.errors.tooManyArgs(args, args.getRaw(0)); }
        ...
        return args.context.arguments
            .slice(...[from, to].sort())
            .map(this.bbtag.convert.toString)
            .join(' ');
    }
}
