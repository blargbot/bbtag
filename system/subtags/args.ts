...
export class ArgsSubtag extends SystemSubtag {
    ...
    public getArgs(args: ArgumentCollection): SubtagResult {
        const from = bbUtil.convert.toNumber(args.get(0), NaN);
        const to = args.length === 1 ? from + 1 : bbUtil.convert.toNumber(args.get(1), v => v === 'n' ? Infinity : NaN);

        if (isNaN(from)) { return bbUtil.errors.types.notNumber(args, args.getRaw(0)); }
        if (isNaN(to)) { return bbUtil.errors.types.notNumber(args, args.getRaw(1)); }

        if (from >= args.context.arguments.length) { return bbUtil.errors.tooManyArgs(args, args.getRaw(0)); }
